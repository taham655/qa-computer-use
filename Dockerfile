...
RUN cd $HOME/frontend && npm install && npm run build

# setup desktop env & app
COPY --chown=$USERNAME:$USERNAME image/ $HOME
COPY --chown=$USERNAME:$USERNAME computer_use_demo/ $HOME/computer_use_demo/

# Frontend is already built at $HOME/frontend/dist (from earlier npm build step)
# The API will look for it at $HOME/frontend/dist

# Environment variables for RoverQA (can be overridden at runtime)
ENV APP_MODE=demo
