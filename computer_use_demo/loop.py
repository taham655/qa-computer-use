    ...
    while True:
        ...
        for content_block in response_params:
            ...
            if (
                ...
                # Handle both sync and async callbacks
                if asyncio.iscoroutinefunction(tool_output_callback):
...
def _response_to_params(
    ...
    for block in response.content:
        if isinstance(block, BetaTextBlock):
            ...
            # Handle tool use blocks - only include accepted fields to avoid
            # sending back extra fields like 'caller' that the API returns but doesn't accept
            res.append(
                cast(
                    BetaToolUseBlockParam,
                    {
